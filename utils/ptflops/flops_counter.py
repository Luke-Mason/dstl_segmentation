...
def flops_to_string(flops, units='GMac', precision=5):
    if units is None:
        if flops // 10**9 > 0:
...
def params_to_string(params_num):
    if params_num // 10 ** 6 > 0:
        return str(round(params_num / 10 ** 6, 5)) + ' M'
    elif params_num // 10 ** 3:
        return str(round(params_num / 10 ** 3, 5)) + ' k'
    else:
        return str(params_num)


def print_model_with_flops(model, units='GMac', precision=5, ost=sys.stdout):
